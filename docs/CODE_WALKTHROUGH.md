...
HTML form
  ...
  -> fetch sends form URL-encoded data to the Java server (local or hosted)
...
## Render and Gmail hosting addendum

`StudentStore.java` is unchanged by the hosting update: arrays remain the core storage and Java still computes every average and highest/lowest flag. The HTTP adapter now reads `PORT`, binds to `0.0.0.0` on Render, and accepts only configured website hosts and browser origins. `/health` is a data-free endpoint for hosting checks. Local use without public configuration still binds to `127.0.0.1`.

The server includes its canonical public HTTPS URL in each report response. `web/share.js` builds a Gmail compose link from that address, and `web/app.js` updates the sharing controls. The email does not contain a snapshot of student names or scores and is not automatically sent.

The public version continues to use one shared in-memory store. Every visitor can view, add or reset the records. There is no login or permanent saving, and only fictional student information should be used. For the actual deployment settings, see `DEPLOY_RENDER.md`. The source package alone does not create a live website.
