...
class BrowserTests(unittest.TestCase):
    ...
    def load_page(self):
        ...
        html = re.sub(r'<script src="(?:app|share)\.js" defer></script>', '', html)
        ...
        self.page.add_script_tag(content=(ROOT / "web" / "share.js").read_text(encoding="utf-8"))
    ...
    def test_local_link_is_not_shareable(self):
        self.ready()
        expect(self.page.locator("#gmail-share")).to_have_attribute("aria-disabled", "true")
        self.assertIsNone(self.page.locator("#gmail-share").get_attribute("href"))
        expect(self.page.locator("#copy-link-btn")).to_be_disabled()

    def test_public_sharing_controls_and_no_records_in_email(self):
        self.add(name="Private Test Name")
        expect(self.page.locator("#student-count")).to_have_text("1")
        self.page.evaluate("updateSharing('https://scoredesk.example/', true)")
        link = self.page.locator("#gmail-share")
        expect(link).to_have_attribute("aria-disabled", "false")
        expect(link).to_have_attribute("target", "_blank")
        self.assertIn("noopener", link.get_attribute("rel"))
        self.assertIn("mail.google.com", link.get_attribute("href"))
        self.assertNotIn("Private", link.get_attribute("href"))
        expect(self.page.locator("#share-url")).to_have_value("https://scoredesk.example/")
        expect(self.page.locator("#demo-warning")).to_be_visible()
        self.page.emulate_media(media="print")
        expect(self.page.locator("#gmail-share")).to_be_hidden()
        expect(self.page.locator("#demo-warning")).to_be_hidden()

    def test_copy_fallback_selects_visible_link(self):
        self.ready()
        self.page.evaluate("""() => {
            updateSharing('https://scoredesk.example/', true);
            Object.defineProperty(navigator, 'clipboard', {configurable: true,
                value: {writeText: async () => { throw new Error('Clipboard disabled'); }}});
        }""")
        self.page.locator("#copy-link-btn").click()
        expect(self.page.locator("#feedback")).to_contain_text("Copy the selected website address")
        self.assertEqual(self.page.locator("#share-url").evaluate("el => el.selectionEnd - el.selectionStart"),
                         len("https://scoredesk.example/"))
