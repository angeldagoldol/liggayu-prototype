...
function renderReport(snapshot) {
  report = snapshot;
  updateSharing(snapshot.publicUrl, snapshot.publicDemo);
...
async function requestReport(path, method = "GET", values = null) {
  ...
  try {
    ...
    if (error instanceof ApiError) {
      ...
      showFeedback("Cannot reach the Java server. A free hosted service may be waking up. Wait a moment and choose Refresh report. For local use, keep the Java terminal open." + actionNote, "error");
    }
    return false;
  } finally {
...
function updateSharing(publicUrl, isPublicDemo) {
  shareUrl = ScoreDeskSharing.publicWebsiteUrl(publicUrl);
  const gmail = byId("gmail-share");
  const composeUrl = ScoreDeskSharing.gmailUrl(shareUrl);
  gmail.setAttribute("aria-disabled", String(!composeUrl));
  gmail.tabIndex = composeUrl ? 0 : -1;
  if (composeUrl) gmail.href = composeUrl;
  else gmail.removeAttribute("href");
  byId("copy-link-btn").disabled = !shareUrl;
  byId("share-url").value = shareUrl || "";
  byId("share-url").hidden = !shareUrl;
  byId("demo-warning").hidden = !isPublicDemo;
  byId("share-note").textContent = shareUrl
    ? "Opens Gmail with the website link only. Add a recipient, review the email, then send."
    : "Gmail sharing becomes available after deployment to your public HTTPS address.";
}

byId("gmail-share").addEventListener("click", (event) => {
...
byId("copy-link-btn").addEventListener("click", async () => {
  ...
  try {
    ...
    showFeedback("Website link copied. Paste it into Gmail or another message.");
  } catch {
    byId("share-url").focus();
    byId("share-url").select();
    showFeedback("Copy the selected website address, then paste it into Gmail.");
  }
});
